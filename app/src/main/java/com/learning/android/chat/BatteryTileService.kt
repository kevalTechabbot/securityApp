...
class BatteryTileService : TileService() {
    ...
    private fun setBatteryInfo(intent: Intent?) {
        ...
        if (isPluggedIn && getSharedPreferences(
            ...
            when (plugState) {
                BatteryManager.BATTERY_PLUGGED_AC -> qsTile.icon =
                    Icon.createWithResource(this, R.drawable.full_battery)
        ...
        if (isFullyCharged) {
            ...
            qsTile.icon = Icon.createWithResource(this, R.drawable.full_battery)
        } else if (isCharging) {
            val customTileText =
                getSharedPreferences("preferences", MODE_PRIVATE).getString("charging_text", "")
            if (!isTappableTileEnabled) qsTile.state = getTileState(true)
            ...
            qsTile.icon = Icon.createWithResource(this, R.drawable.charging_battery)
        } else {
            val customTileText =
            ...
            if (batteryLevel < 10)
                qsTile.icon = Icon.createWithResource(this, R.drawable.empty_battery)
            else
                qsTile.icon = Icon.createWithResource(this, R.drawable.partial_battery)
        }
